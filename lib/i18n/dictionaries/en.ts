...
export const en: Dictionary = {
  ...
  nav: {
    ...
    blurbs: {
      home: "The start page",
      chat: "Paste a message, get it explained, reply",
      situations: "Real sentences from daily life, work and care",
      grammar: "The rules you hear but don’t know",
      dialect: "Zurich, Bern, Basel, Valais and the others",
      vocabulary: "The words you hear at the lunch table",
      speaking: "Say it yourself, with feedback",
      practice: "Short exercises with explanations",
      listen: "Podcasts, shows and series in dialect",
      essays: "Background and stories",
      paper: "How Heidi is built, in detail",
      roadmap: "What comes next",
      changelog: "What changed recently",
      method: "What learning here is based on",
      technology: "Models, checks and privacy",
      contribute: "Build and have a say",
      about: "Who is behind Heidi",
      organisations: "Heidi for teams, care homes and hospitals",
    },
    closeMenu: "Close menu",
    quickChat: "Chat with Heidi",
    quickPractice: "Practise now",
  },
  ...
  model: {
    ...
    pasteHint: "Paste a key — Heidi checks it at once and shows which models it can use.",
    strongest: "strongest",
    vendorSaid: "The provider says:",
  ...
  pillars: [
    ...
    },
    {
      ...
      body: "No points, no invented percentages. The number we want to show you is how much of an unfamiliar Zurich speaker you understand — before and after.",
    },
  ],
  ...
  research: {
    ...
    decisions: [
      ...
      "Measured rather than gamified. No points, no invented numbers.",
  ...
  sync: {
    ...
    body: "Your practice, your streak and your saved words are kept in step between your signed-in devices. Switched off, everything stays in this browser only.",
    signInFirst: "Sign in to have your progress on every device.",
    turnOn: "Turn on",
    turnOff: "Turn off",
    on: "On — this browser syncs with your other devices.",
    deleteAll: "Delete everything synced",
    deleted: "Deleted. Nothing of your progress is left on the server.",
    failed: "That didn’t work. Please try again.",
  },
  certificate: {
    title: "Certificate",
    get: "Issue certificate",
    getting: "Checking …",
    view: "View certificate",
    syncFirst: "Sign in and switch on «Progress on every device» in settings. The certificate is issued on the server from your synced progress, not from this browser.",
    notYet: "The server has {held} of {total} lines secure so far — sync is catching up. Please try again in a minute.",
    failed: "That didn’t work. Please try again.",
    holder: "Whoever holds this certificate",
    statement: "has, in the situation «{scene}», repeatedly understood every one of the {total} lines Heidi asks about.",
    measured: "{held} of {total} lines secure, {stuck} of them again after a break.",
    issued: "Issued on {date}",
    idLabel: "Certificate no.",
    method: "How Heidi checks",
    scope: "A certificate of listening comprehension in one situation — not a language diploma.",
    nameLabel: "Name for printing (not stored)",
    print: "Print",
    notFound: "This certificate does not exist.",
  },
  team: {
    title: "Team",
    lead: "Choose what your team works on. Members decide for themselves whether to show you how far they are in those situations — never which lines, or what they got wrong.",
    focusLabel: "Focus",
    none: "No team — a study group",
    overviewEmpty: "Nobody in the team shows you their progress yet. Each member can switch it on on the group page.",
    private: "does not share",
    noSync: "shares, but has not switched on «Progress on every device» — nothing to see yet",
    ready: "{n} of {total} situations secure",
    certificates: "Certificates: {n}",
    new: "new",
    met: "started",
    steady: "mostly",
    sure: "secure",
    shareTitle: "Your progress in the team",
    shareBody: "Shows {name} how far you are in the team’s situations — never which lines, or what you got wrong. «Progress on every device» must be on in settings.",
    shareOn: "Share with {name}",
    shareOff: "Stop sharing",
    sharing: "You share your progress.",
    failed: "That didn’t work. Please try again.",
  },
  streak: {
    title: "Your streak",
  ...
  practice: {
    ...
    note: "What you keep stays in your browser — unless you switch on «Progress on every device» in settings. The questions from the vocabulary need no account.",
    start: "Start",
    restart: "Again",
    ...
    ask: {
      ...
      reply: "Which reply fits?",
      gist: "What is meant?",
      transform: "Which sentence says exactly this?",
      clock: "Which number is that?",
     ...
     */
    lessons: {
      "false-friend": "«{word}» looks like a German word you know — but it does not mean «{trap}», it means «{means}». These are the words people misunderstand without ever noticing.",
      "word": "«{word}» means «{means}». Keep it with its sentence, not on its own: a word recognised in context is a word you will recognise in conversation.",
      "clock-half": "«halbi drüü» is «halb drei»: the half hour BEFORE the hour named, so 2:30 — not half past three, as «half three» would be in English. The hour often takes an -i: vieri, achti, zwölfi.",
      "clock-quarter": "In a time, «ab» means «nach» (past): Viertel ab drüü is 3:15. «vor» stays «vor»: Viertel vor drüü is 2:45.",
      "price": "Francs first, then rappen, with no «und» in between: drüü Franke zwänzg is 3.20. The tens end in -zg: zwänzg, drissg, vierzg, füfzg.",
      "either-or": "A question with «oder» wants one of its two options back — not a yes or a no.",
      "yes-no": "When the verb comes first (Händ Sie …? Sind Sie …? Bruuched Sie …?), it is a yes-no question. The fitting reply starts with Ja or Nei — or gives exactly what was asked for.",
      "offer": "«Wänd Sie …?» is «Möchten Sie …?» and «Dörf s no öppis sii?» is «Darf es noch etwas sein?» — an offer. You accept or decline it, with a merci.",
      "request": "«Chönd Sie …?» and «Chasch du …?» are questions in form and requests in meaning. The reply agrees or declines; it does not give information.",
      "question-word": "The question word decides: Wie (how), Wie lang (how long), Wänn (when), Wo (where), Was (what), Wer (who), Weli (which). Listen for it first — the reply has to fill exactly that gap.",
      "past": "Zurich German has no simple past. «war» is «isch gsi», «ging» is «isch gange», «sagte» is «hät gseit». Wait for a «war» and you miss that it is already over.",
      "since": "«sit» is «seit» (since): it asks how long something has been so. «für drüü Täg» answers a different question — how long it should last.",
      "indirect-no": "In Zurich people rarely refuse outright. «Mer luegt dänn», «Das müesst mer no aaluege» or «Villicht es anders Mal» are usually the answer already — politely wrapped.",
      "mag": "In Zurich German «möge» often means having the strength or feeling up to something, not only liking it. «Ich mag nüme» means: I can't go on.",
      "es-haet": "«Es hät» often means «es gibt» (there is): Es hät Lüüs — there are lice. Hear «es hat» and you look in vain for whoever has something.",
      "person": "Listen for the person: mir = wir, Chömed er = kommt ihr, si = sie, mer = man or wir, Ihne = Ihnen. In the plural the verb has one form for all three persons, so the person lives only in the pronoun. The polite imperative ends in -ed with Sie (Blibed Sie); the du form does not (Blib).",
      "verb-frame": "The frame around the verb carries the meaning: «isch am warte» is right now, «go» is going there, «cho» coming here, «hät gwartet» is over.",
      "small-word": "One small word decides the whole sentence — it is shown above. These are exactly the words one misses, because in Standard German they sound different or do not exist.",
      "whose": "Zurich German says «wo» for der, die, das in a relative clause (De Maa, wo dört staht) and «em Herr Meier sis Zimmer» for Herrn Meiers Zimmer. Listen for WHO is meant.",
      "thanks-sorry": "The reply reacts to what happened: a favour gets thanks, a complaint an apology, flowers a vase. The participle says what happened: aagnoh, gleert, mitbrocht.",
      "comfort": "Somebody who is afraid, in pain or not up to it needs an answer to exactly that first — not information about something else.",
    },
    ...
    why: [
      ...
      {
        claim: "No points, no percentage — and a streak without reproach.",
        detail:
          "Points measure how much Heidi you consumed while looking like a measure of learning. The streak only counts days you practised and never ends with «lost». What is here are counts of what you did.",
        source: ["yang-2021"],
      },
    ],
  ...
  organisations: {
    ...
    teamsTitle: "Heidi for Teams",
    teamsBody: "Create a group for your team and choose its focus — the situations in a care home, for example. Your people practise on their own devices and decide for themselves whether to show you which situations they are secure in. Whoever masters a situation gets a certificate for it, which you can check through its link.",
    teamsCta: "Create a team",
    talk: "Write to us",
  },
  ...
  situations: {
    ...
    verified: "Every line is machine-checked against Zurich forms: forms from other dialects or from Germany do not get through.",
    hear: "You hear this",
    say: "You say this",
    linesLabel: "lines",
