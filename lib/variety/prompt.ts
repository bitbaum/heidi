...
export function systemPrompt(pack: VarietyPack, explainIn = "English"): string {
  ...
  return [
    ...
    '  "next": [',
    '    { "id": "reply" },',
    '    { "id": "rephrase", "axis": "shorter | warmer | firmer | formal | casual | simpler" }',
    "  ],",
    ...
    "",
    "Omit \"next\" entirely when nothing obvious follows. A button that does not fit",
    "costs more than the blank space it filled.",
  ]
    .filter(Boolean)
    .join("\n");
}
