...
test("a finding a listener would notice comes back with its fix", async () => {
  ...
  const result = await checkGrammar(text, "de-CH", { fetchImpl: async () => lt([agreement(7, 8, "das Buch")]) });
  assert.ok(result);
...
test("what the recogniser decided is never billed to the speaker", async () => {
  ...
  const result = await checkGrammar(text, "de-CH", {
    fetchImpl: async () =>
      lt([
        { offset: 0, length: 3, message: "", replacements: [{ value: "Ich" }], rule: { id: "UPPERCASE_SENTENCE_START", category: { id: "CASING" } } },
        { offset: 10, length: 5, message: "", replacements: [{ value: "Hunger" }], rule: { id: "GERMAN_SPELLER_RULE", category: { id: "TYPOS" } } },
      ]),
  });
...
test("NOT CHECKED IS NOT CLEAN: a down checker is null, never []", async () => {
  ...
  // checker was off.
  assert.equal(await checkGrammar("Ich gehen.", "de-CH", { fetchImpl: async () => new Response("boom", { status: 500 }) }), null);
  assert.equal(
...
test("the default target is loopback, so a transcript does not leave the box", async () => {
  let seen = "";
  const before = process.env.LANGUAGETOOL_URL;
  ...
  try {
    ...
    if (before !== undefined) process.env.LANGUAGETOOL_URL = before;
  }
...
test("at most three are shown, in sentence order, and the total says how many there were", async () => {
  ...
  const result = await checkGrammar(text, "de-CH", {
    fetchImpl: async () => lt([agreement(20, 4, "x"), agreement(0, 4, "x"), agreement(10, 4, "x"), agreement(5, 4, "x")]),
  });
  assert.ok(result);
