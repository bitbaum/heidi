 ...
 *
 * `domain/speaking/delivery.ts` measures the SIGNAL — where the sound was and
...
export function measureFluency(timed: readonly TimedWord[], rule: SyllableRule): Fluency {
  const usable = timed
    .filter((w) => Number.isFinite(w.start) && Number.isFinite(w.end) && w.end >= w.start && w.word.trim().length > 0)
    .slice()
