...
export function markerVerdict(transcript: string, markers: VarietyMarkers): MarkerVerdict {
  ...
  for (const word of words(transcript)) {
    ...
    if (target.has(w)) targetHits.push(w);
    if (bridge.has(w)) bridgeHits.push(w);
  }

  const difference = targetHits.length - bridgeHits.length;
  const variety =
    difference >= MARGIN ? "target" : difference <= -MARGIN ? "bridge" : "unclear";
 ...
 */
export function returnsSpokenVariety(verdict: MarkerVerdict): boolean | null {
  if (verdict.variety === "unclear") return null;
