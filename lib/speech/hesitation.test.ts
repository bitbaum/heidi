...
test("a recogniser that leaves a clean gap gives the same answer", () => {
  const gapped = [w("Er", 0.48, 0.74), w("hat", 0.8, 1.2), w("den", 1.25, 1.6), w("Buch", 3.4, 3.9), w("gelesen", 4.0, 4.6)];
  assert.deepEqual(hesitations([{ startMs: 1600, endMs: 3400 }], gapped), [{ before: "Buch", index: 3, ms: 1800 }]);
});
...
test("an ordinary breath is not pointed at, and the line is exactly NOTABLE_PAUSE_MS", () => {
  assert.deepEqual(hesitations([{ startMs: 1000, endMs: 1000 + NOTABLE_PAUSE_MS - 1 }], GROQ_OBSERVED), []);
  assert.equal(hesitations([{ startMs: 1000, endMs: 1000 + NOTABLE_PAUSE_MS }], GROQ_OBSERVED).length, 1);
});
