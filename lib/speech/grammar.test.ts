...
import { SPOKEN_CATEGORIES, fromLanguageTool, worthShowing, type GrammarFinding } from "./grammar.ts";
...
test("a real grammar finding survives with its text taken from the transcript", () => {
  ...
  assert.equal(findings[0].text, "der", "sliced out of the transcript by offset, not echoed from the vendor");
...
test("a response that is not a response at all yields nothing", () => {
  for (const body of [null, undefined, {}, { matches: "no" }, [], 42]) {
    assert.deepEqual(fromLanguageTool(body, TRANSCRIPT), [], `threw or misread on ${JSON.stringify(body)}`);
  }
});
...
test("the allowlist is what a listener would notice, and holds no machine categories", () => {
  for (const banned of ["TYPOS", "CASING", "PUNCTUATION", "TYPOGRAPHY", "STYLE", "REDUNDANCY"]) {
    assert.ok(!SPOKEN_CATEGORIES.includes(banned), `${banned} must never be judged from a transcript`);
  }
