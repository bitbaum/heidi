 ...
 * said.
 *
 * WHY THIS FILE EXISTS. `domain/speaking/delivery.ts` measures where the sound
...
import type { Delivery } from "../domain/speaking/delivery.ts";
import { usable } from "../domain/speaking/delivery.ts";
import { countFilledPauses } from "./fluency.ts";
import { countSyllablesIn, words as splitWords, type SyllableRule } from "./syllables.ts";
...
export function measureSpoken(
  ...
  return {
    ...
    articulationRate:
      rateable && delivery.speechMs > 0 ? round2(syllableCount / (delivery.speechMs / 1000)) : 0,
 ...
 */
export function interpretation(delivery: Delivery, spoken: Spoken): Shape {
  if (!usable(delivery)) return "unclear";
  if (spoken.speechRate <= 0 || spoken.articulationRate <= 0) return "unclear";
