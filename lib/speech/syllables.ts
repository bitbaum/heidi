 ...
 * every rate it feeds.
 */
export function countSyllables(word: string, rule: SyllableRule): number {
  const cleaned = word.toLowerCase().replace(/[^\p{L}]/gu, "");
  if (!cleaned) return 0;

  const vowels = new Set([...rule.vowels]);
  let count = 0;
  let inNucleus = false;
...
export function countSyllablesIn(words: readonly string[], rule: SyllableRule): number {
  let total = 0;
  for (const word of words) total += countSyllables(word, rule);
  return total;
}
...
export function words(text: string): string[] {
  return (text.match(/[\p{L}][\p{L}'’‑-]*/gu) ?? []).map((w) => w.replace(/^['’]+|['’]+$/g, "")).filter(Boolean);
}
